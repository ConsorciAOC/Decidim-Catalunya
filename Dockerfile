...
# The committed package-lock.json is generated on macOS and only records the darwin
# sass-embedded native binary; installing against it on Linux leaves the embedded Sass
# compiler without its platform binary (build crashes with "compiler exited
# unexpectedly"). Drop the lock and install fresh so npm resolves the Linux-specific
# optional deps for the build platform.
