...
module OmniAuth
  module Strategies
    ...
    class IdCatMobil < OmniAuth::Strategies::OAuth2
      ...
      def request_phase
        log(:debug, "In `request_phase`, with params: redirect_uri=>#{callback_url}, options=>#{options.authorize_params}")
        log(:debug, "`request_phase`, redirecting the user to AOC...")
        super
      end
      ...
      def raw_info
        log(:debug, "Access token response was: #{access_token.response}")
        log(:debug, "Performing getUserInfo...")
        unless @raw_info
          response= access_token.get(options.user_info_path)
          result= %i(status headers body).collect  {|m| response.send(m)}
          log(:debug, "getUserInfo response status/headers/body: #{result}")
