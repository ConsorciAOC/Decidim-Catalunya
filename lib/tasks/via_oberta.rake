...
namespace :via_oberta do
  desc "Checks the given credentials against Via Oberta (document_type nif/passport/residence_card/nie, birthdate yyyy/mm/dd)"
  task :check, [:org_id, :document_type, :id_document, :birthdate] => :environment do |_task, args|
    ...
    puts "\nRESPONSE:"
    service= ViaObertaAuthorizationRq.new(api_config(organization)[:username], api_config(organization)[:password], api_config(organization)[:ens])
    rs= service.send_rq(
      birthdate: birthdate,
      document_type: ::ViaObertaAuthorizationConfig::DOCUMENT_TYPE[document_type],
      id_document: id_document
